...
There are three ways for creating a service for each pod. First one is to expose each pod manually with `kubectl`, second one is to use enable externalAccess then run helm install and third one is to use the project called Metacontroller, which enables such functionality.
...
Helm with externalAccess::
+
--

Install Hazelcast with the following commands:

[source, bash]
----
helm repo add hazelcast https://hazelcast-charts.s3.amazonaws.com/
helm repo update
helm install hz-hazelcast --set externalAccess.enabled=true,service.type=LoadBalancer hazelcast/hazelcast
----
--
...
For helm with externalAccess enabled: 
[source, bash]
----
kubectl get service
NAME                      TYPE           CLUSTER-IP       EXTERNAL-IP      PORT(S)          AGE
hz-hazelcast              LoadBalancer   10.219.246.40    35.230.84.127    5701:30443/TCP   4m2s
hz-hazelcast-0-external   LoadBalancer   10.219.255.141   34.145.108.167   5701:30091/TCP   4m7s
hz-hazelcast-1-external   LoadBalancer   10.219.241.203   34.82.71.106     5701:30687/TCP   4m5s
hz-hazelcast-2-external   LoadBalancer   10.219.247.106   35.247.93.190    5701:32452/TCP   4m4s
----

For other two options:
[source, bash]
----
kubectl get service
NAME             TYPE           CLUSTER-IP       EXTERNAL-IP      PORT(S)          AGE
